...
export function DiexListPage() {
  ...
  const query = useQuery({
    queryKey: ["diex", "list", page, debouncedSearch],
    queryFn: () => diexService.list({ page, pageSize: 10, search: debouncedSearch || undefined }),
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="border-none shadow-sm"><CardContent className="p-5"><div className="relative max-w-xl"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input className="pl-9" placeholder="Buscar número, fornecedor, CNPJ ou requisitante..." value={search} onChange={(event) => setSearch(event.target.value)} /></div></CardContent></Card>
      ...
      <Card className="border-none shadow-sm">
        <CardHeader className="flex flex-row items-center justify-between"><CardTitle className="flex items-center gap-2"><FileSignature className="size-5 text-primary" />Documentos cadastrados</CardTitle>{meta && <Badge variant="outline">{meta.totalItems} DIEx</Badge>}</CardHeader>
        <CardContent>
            ...
            <Table>
              ...
              <TableBody>{query.data.items.map((diex) => (
                <TableRow key={diex.id}>
                  <TableCell><p className="font-medium">{diex.diexNumber ?? `DIEX-${diex.diexCode}`}</p><Badge className="mt-1" variant={diex.diexNumber && diex.issuedAt ? "default" : "secondary"}>{diex.diexNumber && diex.issuedAt ? "Pronto" : "Aguardando SALC"}</Badge></TableCell>
                  ...
                  <TableCell className="text-right"><Button asChild size="sm" variant="outline"><Link to={`/diex/${diex.id}`}>Detalhes</Link></Button></TableCell>
                </TableRow>
              ))}</TableBody>
            </Table>
