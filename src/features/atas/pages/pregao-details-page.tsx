...
export function PregaoDetailsPage() {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <Card><CardContent className="pt-6"><p className="text-sm text-muted-foreground">Valor consumido total</p><p className="mt-1 text-xl font-semibold">{money(pregao.metrics.consumedAmount)}</p></CardContent></Card>
    </div>
    <Card><CardHeader><CardTitle>Atas de Registro de Preços</CardTitle></CardHeader><CardContent>
      {pregao.atas.length ? <div className="overflow-x-auto"><Table className="min-w-[1080px]"><TableHeader><TableRow><TableHead className="w-40">ATA</TableHead><TableHead className="min-w-72">Fornecedor</TableHead><TableHead className="min-w-40">Cobertura</TableHead><TableHead className="min-w-52">Vigência</TableHead><TableHead className="w-20">Itens</TableHead><TableHead className="w-28">Status</TableHead><TableHead className="w-36 text-right">Ação</TableHead></TableRow></TableHeader><TableBody>{pregao.atas.map((ata) => <TableRow key={ata.id}><TableCell><p className="font-semibold">{ata.number}</p><p className="mt-1 text-xs text-muted-foreground">ATA-{ata.ataCode}</p></TableCell><TableCell className="whitespace-normal leading-5">{ata.vendorName}</TableCell><TableCell className="whitespace-normal">{[...new Set(ata.coverageGroups.map((group) => group.name))].join(", ") || "—"}</TableCell><TableCell className="whitespace-nowrap">{formatAtaDate(ata.validFrom)} até {formatAtaDate(ata.validUntil)}</TableCell><TableCell>{ata._count?.items ?? 0}</TableCell><TableCell><Badge variant={ata.isActive ? "default" : "secondary"}>{ata.isActive ? "Vigente" : "Inativa"}</Badge></TableCell><TableCell className="text-right"><div className="flex justify-end gap-1 whitespace-nowrap"><Button asChild variant="ghost" size="sm"><Link to={`/atas/${ata.id}`}>Abrir ATA</Link></Button>{canManage && <Button variant="ghost" size="icon" title="Excluir ATA" onClick={() => setDeleteAta(ata)}><Trash2 className="size-4 text-destructive" /></Button>}</div></TableCell></TableRow>)}</TableBody></Table></div> : <p className="py-10 text-center text-sm text-muted-foreground">Nenhuma ATA vinculada a este pregão.</p>}
    </CardContent></Card>
