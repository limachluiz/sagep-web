...
export function ReportsPage() {
  ...
  return <div className="space-y-6">
    ...
    <Card className="border-none shadow-sm"><CardHeader><CardTitle className="flex items-center gap-2"><FileChartColumn className="size-5 text-primary" />Dossiê individual</CardTitle><CardDescription>Selecione um projeto na prévia ou localize-o na lista completa.</CardDescription></CardHeader><CardContent className="space-y-5"><div className="flex flex-col gap-3 sm:flex-row"><Select value={projectId} onValueChange={setProjectId}><SelectTrigger className="sm:max-w-xl"><SelectValue placeholder="Selecione o projeto" /></SelectTrigger><SelectContent>{projects.map((project) => <SelectItem key={project.id} value={project.id}>PRJ-{project.projectCode} · {project.title}</SelectItem>)}</SelectContent></Select><Button onClick={() => pdfMutation.mutate()} disabled={!projectId || pdfMutation.isPending}><Download className="size-4" />{pdfMutation.isPending ? "Gerando PDF..." : "Baixar dossiê PDF"}</Button></div>
