...
export function IntegrationsSettingsPage() {
  ...
  const removeTokenMutation = useMutation({ mutationFn: systemSettingsService.removePortalApiToken, onSuccess: (data) => { queryClient.setQueryData(["system-settings"], (current: typeof query.data) => current ? { ...current, portalApiToken: data.portalApiToken } : current); setPortalToken(""); toast.success(data.portalApiToken.configured ? "Token armazenado removido; o token do ambiente voltou a ser utilizado." : "Token removido.") }, onError: (error) => toast.error(error.message) })
  ...
  const removePortalToken = () => {
    if (window.confirm("Remover o token armazenado do Portal da Transparência?")) removeTokenMutation.mutate()
  }
