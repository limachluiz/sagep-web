...
export function ServiceOrderDetailsPage() {
  ...
  const canCancel = useAuthStore((state) => state.hasPermission("service_orders.cancel"))
  const canRestore = useAuthStore((state) => state.hasPermission("service_orders.restore"))
  const canDelete = useAuthStore((state) => state.hasPermission("service_orders.delete"))
  const canEdit = useAuthStore((state) => state.hasPermission("service_orders.issue"))
  const [loading, setLoading] = useState<"html" | "pdf" | null>(null)
  const [archiveDialogOpen, setArchiveDialogOpen] = useState(false)
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false)
  const [editOpen, setEditOpen] = useState(false)
  const [registerSignatureOpen, setRegisterSignatureOpen] = useState(false)
  const query = useQuery({ queryKey: ["service-orders", "details", serviceOrderId, includeArchived], queryFn: () => serviceOrdersApi.details(serviceOrderId, includeArchived), enabled: Boolean(serviceOrderId) })
  ...
  return <div className="space-y-6"><div><Button asChild variant="ghost" className="mb-3 -ml-3"><Link to="/service-orders"><ArrowLeft className="size-4" />Voltar às OS</Link></Button><div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end"><div><div className="mb-3 flex gap-2"><Badge>{order.serviceOrderNumber}</Badge>{order.isEmergency && <Badge variant="destructive">Emergencial</Badge>}{order.archivedAt && <Badge variant="outline">Arquivada</Badge>}</div><h1 className="text-3xl font-semibold">Ordem de Serviço</h1><p className="mt-2 text-sm text-muted-foreground">Emitida em {date(order.issuedAt)} · NE {order.commitmentNoteNumber}</p></div><div className="flex flex-wrap gap-2">{canEdit && !order.archivedAt && <Button variant="outline" onClick={() => setEditOpen(true)}><Pencil className="size-4" />Editar</Button>}{canCancel && !order.archivedAt && <Button variant="outline" className="text-destructive hover:text-destructive" onClick={() => setArchiveDialogOpen(true)}><Archive className="size-4" />Arquivar</Button>}{canRestore && order.archivedAt && <Button variant="outline" onClick={() => setArchiveDialogOpen(true)}><RotateCcw className="size-4" />Restaurar</Button>}{canDelete && order.archivedAt && <Button variant="destructive" onClick={() => setDeleteDialogOpen(true)}><Trash2 className="size-4" />Excluir</Button>}<Button variant="outline" disabled={Boolean(loading) || Boolean(order.archivedAt)} onClick={() => document("html")}><ExternalLink className="size-4" />Visualizar</Button><Button disabled={Boolean(loading) || Boolean(order.archivedAt)} onClick={() => document("pdf")}><FileText className="size-4" />Visualizar PDF</Button></div></div></div>
    ...
    <Card className="border-none shadow-sm"><CardHeader><CardTitle className="flex items-center gap-2"><FileSignature className="size-5 text-primary" />OS assinada</CardTitle></CardHeader><CardContent>{order.project.signedServiceOrderLink ? <div className="rounded-xl border border-primary/20 bg-primary/5 p-4"><p className="font-medium">Documento recebido em {date(order.project.signedServiceOrderReceivedAt)}</p>{order.project.signedServiceOrderRegisteredBy && <p className="mt-1 text-xs text-muted-foreground">Registrado por {order.project.signedServiceOrderRegisteredBy.name}</p>}{order.project.signedServiceOrderNotes && <p className="mt-3 text-sm">{order.project.signedServiceOrderNotes}</p>}<Button asChild variant="outline" size="sm" className="mt-4"><a href={order.project.signedServiceOrderLink} target="_blank" rel="noreferrer">Abrir OS assinada<ExternalLink className="size-4" /></a></Button></div> : <div className="rounded-xl border border-dashed p-5"><p className="font-medium">{order.project.serviceOrderSignatureRequired ? "Aguardando documento assinado" : "Registro legado"}</p><p className="mt-1 text-sm text-muted-foreground">{order.project.serviceOrderSignatureRequired ? "A execução permanecerá bloqueada até o registro do link e da data de recebimento." : "Esta OS foi emitida antes da obrigatoriedade do vínculo digital."}</p></div>}</CardContent></Card>
    <Card className="border-none shadow-sm"><CardHeader><CardTitle>Itens ordenados</CardTitle></CardHeader><CardContent><DocumentItemsTable containerLabel="Itens da Ordem de Serviço" items={order.items.map((item) => ({ id: item.id, code: item.itemCode, description: item.description, unit: item.supplyUnit, quantity: quantity(item.quantityOrdered), unitPrice: money(item.unitPrice), totalPrice: money(item.totalPrice) }))} /></CardContent></Card>
    {editOpen && <EditServiceOrderDialog order={order} open={editOpen} onOpenChange={setEditOpen} onSaved={(updated) => { queryClient.setQueryData(["service-orders", "details", serviceOrderId, includeArchived], updated); queryClient.invalidateQueries({ queryKey: ["service-orders", "list"] }); queryClient.invalidateQueries({ queryKey: ["projects"] }) }} />}
    {registerSignatureOpen && <RegisterSignedServiceOrderDialog projectId={order.project.id} projectCode={order.project.projectCode} serviceOrderIssuedAt={order.issuedAt} open={registerSignatureOpen} onOpenChange={setRegisterSignatureOpen} onSaved={() => { queryClient.invalidateQueries({ queryKey: ["service-orders"] }); queryClient.invalidateQueries({ queryKey: ["projects"] }); queryClient.invalidateQueries({ queryKey: ["dashboard"] }) }} />}
    <ArchiveActionDialog open={archiveDialogOpen} onOpenChange={setArchiveDialogOpen} mode={order.archivedAt ? "restore" : "archive"} entityLabel="Ordem de Serviço" entityCode={order.serviceOrderNumber} description={order.archivedAt ? "A OS voltará ao fluxo ativo do projeto." : "O projeto retornará à etapa de liberação da OS. A ação é permitida somente antes do início da execução."} pending={archiveMutation.isPending || restoreMutation.isPending} onConfirm={() => order.archivedAt ? restoreMutation.mutate() : archiveMutation.mutate()} />
    <DeleteActionDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen} entityLabel="Ordem de Serviço" entityCode={order.serviceOrderNumber} description="A OS será removida das listagens ativas e arquivadas." pending={deleteMutation.isPending} onConfirm={() => deleteMutation.mutate()} />
  </div>
}
