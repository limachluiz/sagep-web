...
export function CreateServiceOrderDialog({ details, open, onOpenChange, onCreated }: Props) {
  ...
  const [hasProjectInspector, setHasProjectInspector] = useState(false)
  const [projectInspectorName, setProjectInspectorName] = useState("")
  const [projectInspectorRank, setProjectInspectorRank] = useState("")
  const [projectInspectorCpf, setProjectInspectorCpf] = useState("")
  const [projectInspectorRole, setProjectInspectorRole] = useState("Fiscal do Projeto")
  ...
  const error = useMemo(() => {
    ...
    if (hasProjectInspector && projectInspectorName.trim().length < 3) return "Informe o nome do fiscal do projeto."
    if (hasProjectInspector && projectInspectorRank.trim().length < 2) return "Informe o posto ou graduação do fiscal do projeto."
    if (hasProjectInspector && projectInspectorCpf.replace(/\D/g, "").length !== 11) return "Informe um CPF válido para o fiscal do projeto."
    if (hasProjectInspector && projectInspectorRole.trim().length < 2) return "Informe a função do fiscal do projeto."
    ...
    return null
  }, [contractorCnpj, contractorName, deliveredDocuments, hasProjectInspector, issuedAt, plannedEndDate, plannedStartDate, projectInspectorCpf, projectInspectorName, projectInspectorRank, projectInspectorRole, requesterCpf, requesterName, requesterRank, scheduleItems, selectedDiex])
