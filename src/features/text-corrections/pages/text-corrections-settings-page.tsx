...
export function TextCorrectionsSettingsPage() {
  ...
  const [removeTarget, setRemoveTarget] = useState<TextCorrectionRule | null>(null)
  ...
  const remove = useMutation({
    ...
    onSuccess: () => { toast.success("Correção removida."); setRemoveTarget(null); void refresh() },
    onError: (error) => toast.error(error.message),
  })
  ...
  const applyCatalog = useMutation({
    ...
    onSuccess: (result) => { toast.success(`${result.corrected} de ${result.total} descrição(ões) atualizada(s).`); setApplyOpen(false); void refresh(); void queryClient.invalidateQueries({ queryKey: ["atas"] }) },
    onError: (error) => toast.error(error.message),
  })
  ...
  const applyAlternative = (decision: TextCorrectionDecision, alternative: string) => { setReviewText((current) => current.replace(decision.damagedText, alternative)); setLearnRule({ damagedText: decision.damagedText, correctedText: alternative }) }
  ...
  return <div className="space-y-6">
    ...
    <div className="flex flex-wrap items-start justify-between gap-4">
      ...
      {canManage && <div className="flex gap-2"><Button variant="outline" onClick={() => setApplyOpen(true)} disabled={applyCatalog.isPending}><SpellCheck2 />Processar todas as ATAs</Button><Button onClick={() => openCreate()}><Plus />Nova exceção</Button></div>}
    </div>
    ...
    <Dialog open={Boolean(reviewing)} onOpenChange={(open) => !open && setReviewing(null)}><DialogContent className="sm:max-w-3xl"><DialogHeader><DialogTitle>Revisar descrição do item</DialogTitle><DialogDescription>{reviewing ? `${reviewing.ata.number} · Item ${reviewing.referenceCode}` : "Valide a descrição"}. O texto original continuará preservado.</DialogDescription></DialogHeader>{reviewing && <div className="space-y-4"><div className="rounded-lg border bg-muted/30 p-4"><p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Original recebido</p><p className="mt-2 text-sm leading-6">{reviewing.originalText}</p></div>{reviewing.decisions.filter((decision) => !decision.applied && decision.alternatives.length).map((decision, index) => <div key={`${decision.damagedText}-${index}`} className="rounded-lg border p-4"><p className="text-sm font-medium">Sugestões para <code>{decision.damagedText}</code></p><div className="mt-3 flex flex-wrap gap-2">{decision.alternatives.map((alternative) => <Button key={alternative} type="button" size="sm" variant="outline" onClick={() => applyAlternative(decision, alternative)}>{alternative}</Button>)}</div></div>)}{learnRule && <Alert><BookOpenCheck /><AlertTitle>Aprendizado ativado</AlertTitle><AlertDescription>Ao aprovar, o SAGEP também salvará <code>{learnRule.damagedText}</code> → <code>{learnRule.correctedText}</code> como exceção para futuras importações. <Button type="button" variant="link" className="h-auto p-0" onClick={() => setLearnRule(null)}>Não aprender</Button></AlertDescription></Alert>}<div className="space-y-2"><Label>Descrição final validada</Label><Textarea rows={8} value={reviewText} onChange={(event) => setReviewText(event.target.value)} /></div></div>}<DialogFooter><Button variant="outline" onClick={() => setReviewing(null)}>Cancelar</Button><Button onClick={() => reviewItem.mutate()} disabled={reviewText.trim().length < 3 || reviewItem.isPending}><CheckCircle2 />Aprovar descrição</Button></DialogFooter></DialogContent></Dialog>
    <ConfirmationDialog open={applyOpen} onOpenChange={setApplyOpen} title="Processar todas as descrições?" description="O motor automático será executado em todas as descrições das ATAs. As revisões manuais já realizadas serão preservadas." confirmLabel="Processar descrições" variant="warning" pending={applyCatalog.isPending} onConfirm={() => applyCatalog.mutate()} />
    <ConfirmationDialog open={Boolean(removeTarget)} onOpenChange={(open) => !open && setRemoveTarget(null)} title="Excluir correção personalizada?" description={removeTarget ? `A regra “${removeTarget.damagedText}” → “${removeTarget.correctedText}” será removida e deixará de ser aplicada.` : "A regra será removida."} confirmLabel="Excluir correção" variant="destructive" pending={remove.isPending} onConfirm={() => removeTarget && remove.mutate(removeTarget)} />
  </div>
}
