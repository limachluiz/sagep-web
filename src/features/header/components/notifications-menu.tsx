...
export function NotificationsMenu() {
  ...
  return <DropdownMenu>
    <DropdownMenuTrigger asChild><Button variant="outline" size="icon" className="relative border-primary/15 bg-primary/5 text-muted-foreground hover:bg-primary/10 hover:text-primary" aria-label={`${total} notificações operacionais`}><Bell className="size-4" />{total > 0 && <span className="absolute -right-1.5 -top-1.5 flex min-w-5 items-center justify-center rounded-sm bg-primary px-1 text-[10px] font-bold leading-5 text-primary-foreground shadow-[0_0_12px_rgba(57,255,136,.35)]">{total > 99 ? "99+" : total}</span>}</Button></DropdownMenuTrigger>
