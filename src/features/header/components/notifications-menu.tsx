...
import { AlertTriangle, Bell, Boxes, RefreshCw } from "lucide-react"
...
export function NotificationsMenu() {
  ...
  return <DropdownMenu>
    <DropdownMenuTrigger asChild><Button variant="outline" size="icon" className="relative" aria-label={`${total} notificações operacionais`}><Bell className="size-4" />{total > 0 && <span className="absolute -right-1.5 -top-1.5 flex min-w-5 items-center justify-center rounded-full bg-destructive px-1 text-[10px] font-semibold leading-5 text-white">{total > 99 ? "99+" : total}</span>}</Button></DropdownMenuTrigger>
    <DropdownMenuContent align="end" className="w-[min(92vw,420px)] p-0">
      <DropdownMenuLabel className="flex items-center justify-between px-4 py-3"><span><span className="block text-sm text-foreground">Central de alertas</span><span className="mt-0.5 block font-normal">Atualização automática a cada minuto</span></span><Button variant="ghost" size="icon-sm" onClick={(event) => { event.preventDefault(); query.refetch() }} disabled={query.isFetching} aria-label="Atualizar alertas"><RefreshCw className={query.isFetching ? "size-3.5 animate-spin" : "size-3.5"} /></Button></DropdownMenuLabel>
      ...
      <div className="max-h-[60vh] overflow-y-auto p-2">
        ...
        {alerts.slice(0, 8).map((alert) => <DropdownMenuItem key={alert.id} className="mb-1 cursor-pointer items-start rounded-lg border p-3" onSelect={() => navigate(alert.detailsPath)}><AlertTriangle className={alert.severity === "CRITICAL" ? "mt-0.5 size-4 text-destructive" : "mt-0.5 size-4 text-amber-600"} /><span className="min-w-0 flex-1"><span className="flex items-center gap-2"><span className="truncate font-medium">{alert.title}</span><Badge variant={alert.severity === "CRITICAL" ? "destructive" : "outline"}>{alert.severity === "CRITICAL" ? "Crítico" : "Atenção"}</Badge></span><span className="mt-1 line-clamp-2 text-xs text-muted-foreground">{alert.description}</span></span></DropdownMenuItem>)}
        {stockCount > 0 && <DropdownMenuItem className="cursor-pointer items-start rounded-lg border p-3" onSelect={() => navigate("/atas")}><Boxes className="mt-0.5 size-4 text-amber-600" /><span><span className="block font-medium">{stockCount} item(ns) de ATA com saldo crítico</span><span className="mt-1 block text-xs text-muted-foreground">Abra ATAs e Saldos para revisar a disponibilidade.</span></span></DropdownMenuItem>}
      </div>
