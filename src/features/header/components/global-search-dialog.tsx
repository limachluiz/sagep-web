...
export function GlobalSearchDialog({ compact = false }: { compact?: boolean }) {
  ...
  return <Dialog open={open} onOpenChange={(next) => { setOpen(next); if (!next) setQuery("") }}>
    <DialogTrigger asChild>{compact ? <Button variant="outline" size="icon" className="border-primary/15 bg-primary/5 text-muted-foreground hover:bg-primary/10 hover:text-primary" aria-label="Abrir busca global"><Search className="size-4" /></Button> : <Button variant="outline" className="h-9 w-full justify-start border-border/75 bg-card/55 px-3 font-normal text-muted-foreground hover:border-primary/25 hover:bg-accent/60 hover:text-foreground"><Search className="size-4 text-primary/70" />Buscar em todo o SAGEP...<kbd className="ml-auto hidden rounded border border-border/70 bg-muted px-1.5 py-0.5 font-mono text-[9px] text-muted-foreground xl:inline">CTRL K</kbd></Button>}</DialogTrigger>
