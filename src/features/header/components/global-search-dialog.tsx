...
export function GlobalSearchDialog({ compact = false }: { compact?: boolean }) {
  ...
  return <Dialog open={open} onOpenChange={(next) => { setOpen(next); if (!next) setQuery("") }}>
    <DialogTrigger asChild>{compact ? <Button variant="outline" size="icon" aria-label="Abrir busca global"><Search className="size-4" /></Button> : <Button variant="outline" className="h-9 w-full justify-start px-3 font-normal text-muted-foreground"><Search className="size-4" />Buscar projetos, estimativas, DIEx, OS...</Button>}</DialogTrigger>
    <DialogContent className="max-h-[85vh] gap-4 overflow-hidden p-0 sm:max-w-2xl">
