...
import { CheckCircle2, ChevronDown, ChevronRight, ChevronUp, ChevronsDownUp, ChevronsUpDown, Circle, FileSignature, FileText, Plus, Save, Sparkles, Trash2 } from "lucide-react"
...
export function DeliveryReportBuilder({ projectId, canManage }: { projectId: string; canManage: boolean }) {
  ...
  const toggleExpanded = (current: Set<string>, key: string, update: (next: Set<string>) => void) => { const next = new Set(current); if (next.has(key)) next.delete(key); else next.add(key); update(next) }
  ...
  return <Card className="border-none shadow-sm">
    ...
    <CardContent className="space-y-6">
      ...
      <div className="space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-semibold">Blocos do relatório</h3><p className="text-sm text-muted-foreground">Os modelos já vêm preenchidos. Expanda somente o bloco que deseja revisar ou editar.</p></div><div className="flex flex-wrap gap-2"><Button variant="ghost" size="sm" onClick={() => setExpandedSections(new Set(draft.sections.map((section) => section.key)))}><ChevronsUpDown />Expandir todos</Button><Button variant="ghost" size="sm" onClick={() => setExpandedSections(new Set())}><ChevronsDownUp />Recolher todos</Button>{canManage && <Button variant="outline" size="sm" onClick={addSection}><Plus />Adicionar bloco</Button>}</div></div>
        {draft.sections.map((section, index) => <div key={section.key} className={`rounded-xl border p-4 ${section.included ? "" : "bg-muted/30 opacity-70"}`}>
          <div className="flex flex-wrap items-start gap-3"><Button type="button" variant="ghost" size="icon-sm" title={expandedSections.has(section.key) ? "Recolher bloco" : "Expandir bloco"} onClick={() => toggleExpanded(expandedSections, section.key, setExpandedSections)}>{expandedSections.has(section.key) ? <ChevronDown /> : <ChevronRight />}</Button><label className="mt-2 flex items-center gap-2 text-sm"><input type="checkbox" checked={section.included} disabled={!canManage} onChange={(event) => updateSection(index, { included: event.target.checked, reviewed: event.target.checked ? section.reviewed : false })} />Incluir</label><Input className="min-w-56 flex-1 font-medium" value={section.title} disabled={!canManage} onChange={(event) => updateSection(index, { title: event.target.value, reviewed: false })} /><div className="flex gap-1">{section.reviewed && <Badge className="mr-1 gap-1"><CheckCircle2 className="size-3" />Revisado</Badge>}<Button type="button" variant="ghost" size="icon-sm" title="Mover para cima" disabled={!canManage || index === 0} onClick={() => moveSection(index, -1)}><ChevronUp /></Button><Button type="button" variant="ghost" size="icon-sm" title="Mover para baixo" disabled={!canManage || index === draft.sections.length - 1} onClick={() => moveSection(index, 1)}><ChevronDown /></Button>{section.key.startsWith("custom-") && <Button type="button" variant="ghost" size="icon-sm" title="Remover bloco" disabled={!canManage} onClick={() => change({ ...draft, sections: draft.sections.filter((_, itemIndex) => itemIndex !== index) })}><Trash2 /></Button>}</div></div>
          {section.included && expandedSections.has(section.key) && <><Textarea className="mt-3 min-h-36" value={section.content} disabled={!canManage} placeholder="Redija o conteúdo técnico deste bloco em linguagem formal e objetiva." onChange={(event) => updateSection(index, { content: event.target.value, reviewed: false })} /><div className="mt-3 flex justify-end"><Button type="button" size="sm" variant={section.reviewed ? "default" : "outline"} disabled={!canManage || !section.content.trim()} onClick={() => updateSection(index, { reviewed: !section.reviewed })}>{section.reviewed ? <CheckCircle2 /> : <Circle />}{section.reviewed ? "Bloco revisado" : "Marcar como revisado"}</Button></div></>}
        </div>)}
      </div>

      <div className="space-y-3"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-semibold">Memória técnica dos itens executados</h3><p className="text-sm text-muted-foreground">O SAGEP analisa cada item e sugere um texto inicial com as características realmente registradas. Todo conteúdo permanece editável.</p></div><div className="flex flex-wrap gap-2"><Button variant="ghost" size="sm" onClick={() => setExpandedItems(new Set(query.data.items.map((item) => item.itemId)))}><ChevronsUpDown />Expandir itens</Button><Button variant="ghost" size="sm" onClick={() => setExpandedItems(new Set())}><ChevronsDownUp />Recolher itens</Button>{canManage && <Button type="button" variant="outline" size="sm" onClick={applySuggestions}><Sparkles />Preencher textos sugeridos</Button>}</div></div>
        {query.data.items.map((item) => { const detailIndex = draft.itemDetails.findIndex((detail) => detail.itemId === item.itemId); const detail = draft.itemDetails[detailIndex]; if (!detail) return null; const expanded = expandedItems.has(item.itemId); return <div key={item.itemId} className="rounded-xl border p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div className="flex max-w-4xl items-start gap-2"><Button type="button" variant="ghost" size="icon-sm" title={expanded ? "Recolher item" : "Expandir item"} onClick={() => toggleExpanded(expandedItems, item.itemId, setExpandedItems)}>{expanded ? <ChevronDown /> : <ChevronRight />}</Button><div><Badge variant="outline">Item {item.itemCode}</Badge><p className="mt-2 text-sm font-medium">{item.description}</p><p className="mt-1 text-xs text-muted-foreground">Origem: {item.sourceQuantity} {item.sourceUnit} · Valor total: {Number(item.totalPrice).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</p></div></div>{detail.technicalDescription.trim() && <Badge className="gap-1"><CheckCircle2 className="size-3" />Documentado</Badge>}</div>{expanded && <div className="mt-4 grid gap-4 sm:grid-cols-[140px_160px_1fr]"><div className="space-y-2"><Label>Unidade no relatório</Label><Input value={detail.unit} disabled={!canManage} placeholder="m ou Und." onChange={(event) => change({ ...draft, itemDetails: draft.itemDetails.map((value, index) => index === detailIndex ? { ...value, unit: event.target.value } : value) })} /></div><div className="space-y-2"><Label>Quantidade executada</Label><Input value={detail.quantity} disabled={!canManage} inputMode="decimal" onChange={(event) => change({ ...draft, itemDetails: draft.itemDetails.map((value, index) => index === detailIndex ? { ...value, quantity: event.target.value } : value) })} /></div><div className="space-y-2"><div className="flex items-center justify-between gap-2"><Label>Descrição técnica aplicada</Label>{canManage && <Button type="button" variant="ghost" size="sm" onClick={() => change({ ...draft, itemDetails: draft.itemDetails.map((value, index) => index === detailIndex ? { ...value, technicalDescription: item.suggestedTechnicalDescription } : value) })}><Sparkles />Usar sugestão</Button>}</div><Textarea className="min-h-36" value={detail.technicalDescription} disabled={!canManage} placeholder={technicalPlaceholder} onChange={(event) => change({ ...draft, itemDetails: draft.itemDetails.map((value, index) => index === detailIndex ? { ...value, technicalDescription: event.target.value } : value) })} /><p className="text-xs text-muted-foreground">Sugestão produzida somente a partir da descrição, unidade e quantidade deste item.</p></div></div>}</div> })}
      </div>
